...
st.set_page_config(
    page_title="Chat with Gym BRO!",
    page_icon=":weight_lifter:",  # Favicon emoji
)
...
st.title("Gym BRO - Fitness & Nutrition Chatbot")

# Display the chat history
for message in st.session_state.chat_session.history:
    with st.expander(translate_role_for_streamlit(message.role), expanded=True):
...
if user_prompt:
    ...
    if is_relevant_question(user_prompt):
        try:
            # Send user's message to Gym BRO and get the response
            gemini_response = st.session_state.chat_session.send_message(user_prompt)
            
            # Display Gym BRO's response
